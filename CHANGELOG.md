...
### Changed
- Repositioned the LLM as an **optional enhancement, not a prerequisite**.
  When delivered, M1 will use the **VS Code Language Model API**
  (GitHub Copilot) instead of a third-party SDK — no extra account, no
  API key, silent fallback to the deterministic answer when Copilot
  isn't available. See [`ROADMAP.md`](./ROADMAP.md) M1.

### Added
- GitHub Actions release pipeline (`.github/workflows/release.yml`):
  pushing a `v*` tag now auto-builds the `.vsix` and publishes a
  GitHub Release with the asset attached.
